...
abstract class EmbyWatchHistoryRemoteDataSource {
  /// 获取 Emby 侧的“继续播放”列表
  Future<List<EmbyResumeItemDto>> getContinueWatching();
...
class EmbyWatchHistoryRemoteDataSourceImpl
  ...
  EmbyWatchHistoryRemoteDataSourceImpl({required EmbyApiClient apiClient})
    : _apiClient = apiClient;

  @override
  Future<List<EmbyResumeItemDto>> getContinueWatching() async {
    try {
      final items = await _apiClient.getContinueWatching();
      return items;
    } catch (error) {
      rethrow;
    }
...
class MockEmbyWatchHistoryRemoteDataSource
    implements EmbyWatchHistoryRemoteDataSource {
  @override
  Future<List<EmbyResumeItemDto>> getContinueWatching() async => const [];
