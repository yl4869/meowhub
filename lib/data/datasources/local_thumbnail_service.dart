...
class LocalThumbnailService {
  LocalThumbnailService({AndroidSafService? safService})
  ...
  Future<String?> _generateThumbnail(String filePath, File thumbFile) async {
    // SAF content:// URI — delegate to native MediaMetadataRetriever.
    if (filePath.startsWith('content://')) {
      final safService = _safService;
      if (safService == null) {
        debugPrint('[LocalMedia][Thumb] SAF service unavailable for: $filePath');
        return null;
      }
      debugPrint('[LocalMedia][Thumb] 尝试 SAF 缩略图生成...');
      final result = await safService.generateThumbnail(
        filePath,
        thumbFile.path,
      );
      if (result != null && await thumbFile.exists()) {
        debugPrint('[LocalMedia][Thumb] SAF 缩略图成功: $result');
        unawaited(_scheduleCleanup());
        return result;
      }
      debugPrint('[LocalMedia][Thumb] SAF 缩略图失败');
      return null;
    }

    // dart:io path — existing logic.
