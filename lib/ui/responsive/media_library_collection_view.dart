...
class MediaLibraryCollectionView extends StatelessWidget {
  const MediaLibraryCollectionView({super.key, required this.mediaType});

  static const String routePath = '/library/:type';

  static String locationFor(MediaType mediaType) =>
      '/library/${mediaType.toValue()}';
  ...
  Widget build(BuildContext context) {
    ...
    final items = switch (mediaType) {
      MediaType.movie => mediaWithUserData.enrichedMovies,
      MediaType.series => mediaWithUserData.enrichedSeries,
    };
    final recentMediaKeys = mediaWithUserData.recentWatching
        .map((item) => item.mediaKey)
        .toSet();

    return Scaffold(
      appBar: AppBar(
        title: Text('全部${mediaType.displayName}'),
                  ...
                  child: Padding(
                    ...
                    child: AppSurfaceCard(
                      child: Column(
                        ...
                        children: [
                          Text(
                            mediaType.collectionTitle,
                            style: Theme.of(context).textTheme.titleLarge,
                          ),
                          const SizedBox(height: 8),
                          Text(
                            '共 ${items.length} ${mediaType.unitLabel}内容，卡片布局会根据屏幕宽度自动调整。',
                ...
                if (mediaWithUserData.isLoading && items.isEmpty)
                  ...
                  SliverFillRemaining(
                    ...
                    child: _CollectionStateCard(
                      ...
                      buttonLabel: '重新加载',
                      onPressed: mediaLibraryProvider.loadInitialMovies,
                    ),
                  )
                  ...
                  SliverFillRemaining(
                    ...
                    child: _CollectionStateCard(
                      ...
                      message: '这里暂时还没有${mediaType.displayName}内容。',
                    ),
                  )
                else
                  SliverPadding(
                    ...
                    sliver: SliverGrid.builder(
                      ...
                      itemBuilder: (context, index) {
                        ...
                        return PosterCard(
                          ...
                          isRecent: recentMediaKeys.contains(
                            mediaItem.mediaKey,
                          ),
...
extension on MediaType {
  String get displayName => this == MediaType.movie ? '电影' : '电视剧';

  String get collectionTitle => this == MediaType.movie ? '电影片库' : '电视剧片库';

  String get unitLabel => this == MediaType.movie ? '部' : '部';
}
