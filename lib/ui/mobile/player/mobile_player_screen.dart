...
class MobilePlayerScreen extends StatefulWidget {
  ...
  final String? autoResolutionLabel;
...
class _MobilePlayerScreenState extends State<MobilePlayerScreen> {
  ...
  Future<void> _handlePlaybackStarted(MeowVideoPlaybackStatus status) async {
    ...
    // warmup+seek 完成后 status.position 可能是解码器从 0 输出了几帧的过渡值，
    // 只有与 initialPosition 偏差在 3 秒以内才认为可靠，否则用 initialPosition
  ...
  Future<void> _performSyncOnExit() async {
    ...
    try {
      ...
      // stopPlayback 失败时退而求其次：至少把最终进度同步到服务器
      try {
        await _udp.syncProgressToServerForItem(
          capturedItem,
          position: snapshot.position,
          duration: snapshot.duration,
          playSessionId: widget.playSessionId,
  ...
  Future<void> _selectResolutionOption(PlayerResolutionOption option) async {
    ...
    _showCenterToast(_resolutionLabel(option));
  }

  String _resolutionLabel(PlayerResolutionOption option) {
    final autoLabel = widget.autoResolutionLabel?.trim();
    if (option.label != 'Auto' || autoLabel == null || autoLabel.isEmpty) {
      return option.label;
    }
    return autoLabel;
  }
  ...
  Future<void> _seekToFraction(double value) async {
    ...
    if (widget.isTranscoding && widget.onServerSeekRequested != null) {
      try {
        ...
        // Ignore server-side seek failures and fall back to local seeking.
      }
    }
    if (p == null) {
      return;
  ...
  Widget _buildPlayer() {
    ...
    return RepaintBoundary(
      ...
      child: MeowVideoPlayer(
        ...
        subtitleStreamIndex:
            widget.subtitleStreamIndexForPlayer ?? widget.subtitleStreamIndex,
