...
- 提供计算属性：enrichedMovies, enrichedSeries, continueWatching, allItems
- 自动处理数据转换和组合逻辑
...
| **UserDataProvider** | 用户个人数据 | favoriteItems, watchHistory, playbackProgress | toggleFavorite(), loadWatchHistory(), updateProgress(), updatePlaybackProgress(), markContinueWatching(), clearPlaybackProgress() |
| **MediaLibraryProvider** | 媒体库加载 | movies, series, isLoading, errorMessage | loadInitialMovies(), refreshMovies(), fetchMovies() |
| **MediaWithUserDataProvider** | 数据组合 | enrichedMovies, enrichedSeries, continueWatching, allItems | 无方法，只提供计算属性 |
